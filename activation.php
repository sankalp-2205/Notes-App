...
<?php
        ...
        include ('connection.php');
            ...
            $sql = "UPDATE users SET activation = 'active' WHERE (email = '$email' and activation = '$key') LIMIT 1";
            if($link-> query($sql))
            {
                if($link-> affected_rows == 1)
                {
                   echo "<div class = 'alert alert-success'>You account has been successfully activated </div>"; 
